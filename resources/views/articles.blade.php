...
<title>Rs. Medistra || Articles</title>
         ...
         <div class="row mt-4">
          ...
          <div class="col-xxl-4 col-xl-4 col-md-4 col-sm-6 col-6 text-capitalize my-3">
            <div class="card shadow">
              <img src="{{ $article['image'] }}" class="card-img-top rounded-3">
              <div class="card-body text-capitalize text-center" style="background-color: whitesmoke; border-radius: 1rem;">
                <h6 class="text-center text-secondary">
                  ...
                  {{ $article->created_at->format("j-m-Y H:i:s") }}
                </h6>
                <h4 class="card-title text-center fs-5 fw-bold my-3">{{ Str::limit($article['title'], 50) }}</h4>
                <p class="text-center fs-6">{{ Str::limit($article['information'], 120) }}</p>
                <a href="/articles/{{ $article['slug'] }}" class="btn btn-success d-flex justify-content-center fw-bold fs-6">selengkapnya</a>
              </div>
            </div>
          </div>
          @endforeach
