         ...
         <div class="row mt-4">
          ...
          <div class="col-xxl-4 col-xl-4 col-md-4 col-sm-6 col-6 text-capitalize my-3">
            <div class="card shadow">
              <img src="{{ $new['image'] }}" class="card-img-top rounded-3">
              <div class="card-body text-capitalize text-center" style="background-color: whitesmoke; border-radius: 1rem;">
                <h6 class="text-center text-secondary">
                  ...
                  {{ $new->created_at->format("j-m-Y H:i:s") }}
                </h6>
                <h4 class="card-title text-center fs-5 fw-bold my-3">{{ Str::limit($new['title'], 50) }}</h4>
                <p class="text-center fs-6">{{ Str::limit($new['information'], 120) }}</p>
                <a href="/news/{{ $new['slug'] }}" class="btn btn-success d-flex justify-content-center fw-bold fs-6">selengkapnya</a>
              </div>
            </div>
          </div>
          @endforeach
